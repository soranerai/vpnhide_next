...
void print_usage(const char *prog) {
    fprintf(stderr, "Usage: %s <targets|direct|port_targets|port_rules|debug|phys> [args...]\n", prog);
    fprintf(stderr, "  port_rules format: <uid> <rule_count> <start> <end> <proto> ...\n");
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "targets") == 0 || strcmp(argv[1], "direct") == 0 || strcmp(argv[1], "port_targets") == 0) {
        data.count = argc - 2;
        if (data.count > MAX_TARGET_UIDS) data.count = MAX_TARGET_UIDS;
        ...
        while (arg_idx < argc && pdata.count < MAX_TARGET_UIDS) {
            ...
            for (int i = 0; i < rules_to_parse; i++) {
                if (arg_idx + 2 >= argc) break;
                target->rules[i].start_port = (unsigned short)atoi(argv[arg_idx++]);
                target->rules[i].end_port = (unsigned short)atoi(argv[arg_idx++]);
                target->rules[i].protocol = (unsigned char)atoi(argv[arg_idx++]);
                target->rule_count++;
            }
            pdata.count++;
        }
