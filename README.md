...
> [!WARNING]
> **Этот форк использует крайне агрессивные методы сокрытия на уровне ядра (kmod) и системного фреймворка.**
...
> В частности, на ядрах с активным механизмом **CFI (Control Flow Integrity)** попытки динамического разрешения символов или перехвата функций могут приводить к немедленному падению системы (Kernel Panic). Также некорректная работа возможна на прошивках с модифицированным или отключённым SELinux. Вы используете это решение исключительно на свой страх и риск.
