...
> [!TIP]
> **Новое решение: built-in режим (WIP)**
> Для достижения абсолютной скрытности, максимальной стабильности и полного исключения накладных расходов была разработана отдельная ветвь проекта. Код сокрытия вшивается непосредственно в монолитное ядро на этапе сборки.
> * **Нулевой модульный оверхед**: логика сокрытия выполняется нативно как часть ядра, полностью исключая задержки вызовов и маршрутизации.
> * **Абсолютная скрытность**: полная неуязвимость к timing-атакам на системные вызовы (syscall), делающая присутствие механизмов защиты необнаружимым. 
> Готовые билды с инструкцией доступы в репо [GKI_KernelSU_SUSFS](https://github.com/soranerai/GKI_KernelSU_SUSFS).
