...
1. **Модуль ядра (LKM или Built-in)**:
   * **LKM (Loadable Kernel Module)**: Поставляется в виде загружаемого модуля ядра для ядер GKI (Android 12–16, версии 5.10–6.12). Использует механизм `kretprobes` для перехвата сетевых вызовов и функций файловой системы.
